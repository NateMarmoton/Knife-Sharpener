...
static SemaphoreHandle_t frameBufferSemaphore;
static QueueHandle_t     vsyncQueue;
...
{
	frameBufferSemaphore = xSemaphoreCreateBinary();
	xSemaphoreGive(frameBufferSemaphore);
	assert((frameBufferSemaphore != NULL) && "Creation of framebuffer semaphore failed");

	vsyncQueue = xQueueCreate(1, sizeof(uint32_t));
	assert((vsyncQueue != NULL) && "Creation of vsync message queue failed");
}
...
{
	xSemaphoreTake(frameBufferSemaphore, 0);
}
...
void OSWrappers::giveFrameBufferSemaphoreFromISR() {
  ...
  xSemaphoreGiveFromISR(frameBufferSemaphore, &xHigherPriorityTaskWoken);
...
void OSWrappers::signalVSync() {
  ...
  xQueueSendToBack(vsyncQueue, &dummy, portMAX_DELAY);
...
void OSWrappers::waitForVSync() { 
  uint32_t dummyGet;
  xQueueReceive(vsyncQueue, &dummyGet, 0);
  // xQueueReceive(vsyncQueue, &dummyGet, 100);
}
